...
//Mockable
{
    ...
    NoteProvider.prototype.getNoteIndex = function (callback) {
        ...
        api.retrieveIndex({
            success: function (resultsArray) {
                console.debug("Found " + resultsArray.length + " notes in index");
                callback(resultsArray);
            },
            error: function (code) {
                console.error("Fatal Error: Failed to get note indices " + code);
                callback(code);
            }
        });
    };
    ...
    NoteProvider.prototype.saveNote = function (body, tag, callback, errorCallback) {
         api.createNote({
            body,
    ...
    NoteProvider.prototype.updateNote = function (key, noteBody, tag, version, deleted, callback, errorCallback) {
    ...
    NoteProvider.prototype.deleteNote = function (key, callback, errorCallback) {
        api.deleteNote({
           key: key,
...
function NoteService(concord) {
    ...
    this.decrementPendingNotes = function () {
        --pendingNotes;
        this.ngScope.setLoadingCountdown(store.notes.length - pendingNotes, store.notes.length);
    }.bind(this);
    ...
    this.saveNote = function () {
        ...
        if (store.note && store.note.key) //If key exists, update
        {
            console.debug("=> Saving/updating note. " + store.note.value.length + " chars.");
            this.np.updateNote(store.note.key, noteText, TAG, null, 0,
                this.parseReceivedNote, this.onNoteActionFailure);
        } else //create new
        {
            console.debug("=> Creating note.");
            this.np.saveNote(noteText, TAG, this.onNoteCreated, this.onNoteActionFailure);
        }
    ...
    this.loadNotes = function (forceRefresh) {
        if ((!store) || !this.isCookieValid() || this.ngScope.isAppDisabled) return;
    ...
    this.loadNote = function (note, noteMetadata) {
        if (note == undefined)
            throw "Note argument not specified";
        ...
        setTimeout(() => {
            this.np.getNote({...note, ...noteMetadata}, this.parseReceivedNote, this.onNoteActionFailure);    
        }, delay); 
    }.bind(this);
    ...
    this.parseNoteIndex = function (outlineNotes) {
        ...
        store.notes = _.filter(store.notes, function (storedNote) {
            return null != _.find(outlineNotes, function (newNote) {
        ...
        outlineNotes.forEach(function (outlineNote) {
            ...
            if (storedNote) {
                if (sn.modifydate === storedNote.modifydate) {
                    if (storedNote.key == store.selectedNoteKey && opIsLoaded() == false)
        ...
        if (updateSelectedNote) {
            this.launchNote(store.note, this.ngScope.saveState != saveStates.saving);
            this.ngScope.finishMainRefresh();
        }
    ...
    this.onNoteActionFailure = function (code, errXHR) {
        ...
        if (errXHR) {
            if (errXHR.status == 401) //errXHR.statusText
                this.killSession(this.m.unauthorized);
            if (errXHR.status == 403)
                this.killSession(this.m.sessionExpired);
            if (errXHR.status == 0)
    ...
    this.parseReceivedNote = function (snote) {
        ...
        if (!saveNote || (!saveNote.modifydate || saveNote.modifydate < snote.modifydate) || snote.deleted == 1) {
            if (snote.deleted == 1) {
                ...
                try {
                    ...
                    forceLaunch = (snote.content != undefined) && ((saveNote.modifydate == undefined) || (saveNote.modifydate != snote.modifydate && store.selectedNoteKey == snote.key));
                    ...
                    store.save();
                    console.log("Saved note: " + saveNote.title + " v" + saveNote.version);
                } catch (error) {
                    console.log('Error occured when trying to save note. ' + error.message)
                }
            }
        }

        if (store.selectedNoteKey && (saveNote && store.selectedNoteKey == saveNote.key)) {
            this.launchNote(saveNote, forceLaunch && this.ngScope.saveState != saveStates.saving);
            this.ngScope.finishMainRefresh();
        }
    ...
    this.launchNote = function (note, useDefault) {
        ...
        try {
            if (useDefault == true || !note.key || ((note && store.note) && store.note.key && note.key != store.note.key)) {
    ...
    this.outlineToXml = function () {
        return this.outliner.op.outlineToXml(null, store.email, null, (store.note) ? store.note.title : null);
    }.bind(this);
...
function NoteStore() {
    ...
    this.load = function () {
        ...
        try {
            ...
            obj.notes.forEach(function (note) {
                var n = new Note(note.value, note.key, note.version, note.modifydate);
                self.notes.push(n);
            });
            ...
            console.error("Error occured when parsing saved data " + err.message)
            return null;
        }
    }
    ...
    this.requiresUpdate = function () {
        if (this.version && this.version >= minVersion)
            return false;
        return true;
    };
...
function Note(v, k, ver, date) {
    ...
    function setTitle() {
        if (val && typeof val == "string") {
            ...
            if (out && out.length > 0) _title = strip(out[0].getAttribute("text"));
            if (_title.length > 30)
...
/* Utils */
{
    ...
    var navigationKeystrokes = [16, 17, 37, 38, 39, 40];
    ...
    $(document).bind('touchend', function (e) {
        ...
        if (!target.is(".divOutlinerContainer") && !target.is("#main") &&
            (isBarTouch == false || target.is(".bar-icon-left"))) return;
...
/* Angular */
{
    ...
    myApp.controller('helloController', ['$scope', 'context', function ($scope, context) {
        $scope.context = context;
    }]);

    myApp.controller('MainCtrl', ['$scope', '$timeout', 'context', function ($scope, $timeout, context) {
        /* Safe Apply */
        {
        ...
        $scope.hideOverlay = function () {
            if ($scope.showLogin || $scope.isAppDisabled)
                return;
            $scope.showOverlay = false;
        };
        ...
        $scope.initialize = function () {
            if (ns.outliner) {
                ...
                nse.addEventListener(nse.textModeChangedEvent.type, concordEvent);
            }
        };
        ...
        /* Login */
        {
            ...
            $scope.isLoginButtonDisabled = function () {
                return ($scope.loginButtonText == loginStates.loggingIn || $scope.loginButtonText == loginStates.loggedIn);
            };
        ...
        /* Working overlay */
        {
            ...
            $scope.setLoadingCountdown = function (loaded, total) {
                if ($scope.showWorking == false) return;
                $scope.loadingCountdownMessage = ' ' + loaded + ' of ' + total;
                $scope.update();
            }
        }
        ...
        /* Save */
        {
            $scope.statusColor = function () {
                ...
                if ($scope.saveState == saveStates.updating) return "yellow";
                if ($scope.saveState == saveStates.failed) return "red";
            };

            $scope.statusOpacity = function () {
                if ($scope.saveState == saveStates.modified || $scope.saveState == saveStates.saving) return 0.5;
                if ($scope.saveState == saveStates.saved) return 1;
            };

            $scope.setSaveState = function (state) {
                $scope.saveState = state; //todo: not called when expansion changed
                $scope.update();
            };

            $scope.forceSync = function () {
                if ($scope.saveState == saveStates.saving || appPrefs.readonly)
                    return;
                else {
        ...
        /* Bar + Bar Menu*/
        {
            $scope.isAppReady = function () {
                return !context.hello.isVisible && !$scope.isAppDisabled && $scope.isLoggedIn();
            };
            ...
            $scope.toggleHello = function () {
                $scope.context.hello.isVisible = !$scope.context.hello.isVisible;
            };

            $scope.toggleBarMenuDialog = function () {
                $scope.showBarMenu = !$scope.showBarMenu;
                if ($scope.showBarMenu)
                    $scope.hideAllBarMenuChildren();
            };
        ...
        /* Touch */
        {
            $scope.isTouchEdit = function () {
                if (ns && ns.outliner) {
                    return !appPrefs.readOnly && $scope.isMobile() && ns.outliner.op.inTextMode();
                }
                return false;
            };
            ...
            $scope.strikethrough = function () {
                if (ns)
                    ns.outliner.op.strikethroughLine();
            }
